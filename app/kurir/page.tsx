    ...
    <CourierLayout>
      <div className="max-w-6xl mx-auto py-2">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <p className="text-xs font-bold text-ecoeat-muted uppercase tracking-widest mb-1">
              CURRENT STATUS
            </p>
            <h1 className="text-4xl font-extrabold text-ecoeat-text mb-1">
              Hello, {user?.name || 'Courier'}
            </h1>
            <p className="text-ecoeat-muted font-medium">
              EcoPay Balance: Rp {(user?.ecoPayBalance || 0).toLocaleString()}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            <div className="bg-[#bce6ef] h-[320px] rounded-[24px] relative overflow-hidden shadow-sm border border-black/5 flex items-center justify-center">
              ...
              <div className="text-[#1e8932] font-bold opacity-70 z-0">[ Map Area ]</div>
            </div>
            <ImpactWidget />
          </div>
        </div>
