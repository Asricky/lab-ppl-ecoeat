...
export default function SellerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: 'Donations', href: '/seller/donations', icon: HeartHandshake },
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 flex-shrink-0 gap-6">
            ...
            <div className="flex items-center space-x-5">
              ...
              <Link href="/seller/wallet" className="bg-[#E8F3EB] px-4 py-1.5 rounded-full flex items-center space-x-2 border border-[#D1E8D7] shadow-sm">
                <Wallet size={16} className="text-[#1A5632]" />
                <span className="text-sm font-bold text-[#1A5632]">Rp {(user?.ecoPayBalance ?? 0).toLocaleString('id-ID')}</span>
              </Link>
              ...
              <div className="relative">
                <button onClick={() => setProfileOpen(!profileOpen)} className="text-gray-400 hover:text-gray-600 transition-colors flex items-center">
