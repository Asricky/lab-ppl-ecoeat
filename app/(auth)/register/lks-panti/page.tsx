...
export default function RegisterLksPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!formData.lksType) {
      showToast("Please select LKS Type", "error");
      return;
    }
    if (!formData.legalPermit.trim()) {
      showToast("Please enter LKS Legal Permit", "error");
      return;
    }
    ...
    if (!formData.foodStorage) {
      showToast("Please select Food Storage type", "error");
      return;
    }
    if (!file) {
      showToast("Please upload your Legal Document / SK Kemenkumham", "error");
      return;
    }
    ...
    <div className="min-h-screen bg-[#f5f7f4] flex flex-col items-center justify-center p-6 relative py-12">
      <div className="text-center mb-8">
        <div className="inline-flex items-center space-x-2 bg-green-100 text-green-800 px-4 py-1.5 rounded-full mb-6">
          ...
          <span className="text-xs font-bold uppercase tracking-wider">Join our Sustainable Network</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
          Register as LKS / Panti Sosial
        </h1>
        <p className="text-gray-500 text-lg">
          Receive surplus food donations from businesses
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                type="email"
                placeholder="contact@yayasan.com"
                className="w-full bg-[#eef1ed] border-transparent rounded-lg px-4 py-3.5 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                required
              >
                <option value="">Select LKS Type</option>
                <option value="Panti Asuhan">Panti Asuhan (Orphanage)</option>
                <option value="Panti Jompo">Panti Jompo (Nursing Home)</option>
                <option value="Panti Wreda">Panti Wreda (Nursing Home)</option>
                <option value="Panti Rehabilitasi">Panti Rehabilitasi (Rehabilitation)</option>
                <option value="Yayasan Sosial">Yayasan Sosial (Social Foundation)</option>
                <option value="Komunitas Sosial">Komunitas Sosial (Social Community)</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">
                Legal Permit (Nomor Izin LKS)
              </label>
              <input
                type="text"
                placeholder="E.g. LKS-DINSOS/3174/2024"
                className="w-full bg-[#eef1ed] border-transparent rounded-lg px-4 py-3.5 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">
                Capacity (Kapasitas Penerima Manfaat)
              </label>
              <input
                type="number"
                placeholder="E.g. 135"
            ...
            <div>
              <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">
                Food Storage Type (Penyimpanan Makanan)
              </label>
              <select
          ...
          <div className="bg-[#eef5ef] border border-green-100 rounded-xl p-4 flex items-start space-x-3 mt-6">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              <span className="font-bold text-green-800">Status note:</span> Your organization will be reviewed before activation. This manual verification ensures the safety and quality of our ecosystem.
            </p>
          </div>
      ...
      <p className="mt-8 text-center text-sm text-gray-500 max-w-md">
        By registering, you agree to our <Link href="#" className="underline hover:text-gray-800">Terms of Service</Link> and <Link href="#" className="underline hover:text-gray-800">Donation Acceptance Policy</Link>.
      </p>
